...
\echo Use "CREATE EXTENSION [ IF NOT EXISTS ] scharp_pg_dba_tools WITH SCHEMA tools [ CASCADE ]" to load this file. \quit

CREATE SCHEMA IF NOT EXISTS @extschema@;

GRANT USAGE ON SCHEMA @extschema@ TO PUBLIC;

CREATE OR REPLACE FUNCTION @extschema@.pg_stat_progress_vacuum ()
RETURNS TABLE (
  database_name name,
  schema_name name,
  table_name name,
...
GRANT EXECUTE ON FUNCTION @extschema@.pg_stat_progress_vacuum () TO PUBLIC;

CREATE OR REPLACE FUNCTION @extschema@.planning_time (
  queries text []
)
RETURNS TABLE (
  query text,
...
GRANT EXECUTE ON FUNCTION @extschema@.planning_time (queries text []) TO PUBLIC;
